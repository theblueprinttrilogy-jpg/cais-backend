#!/usr/bin/env python3
"""
CAIS REAL PIPELINE - Fixed version
Handles empty folders, non-downloadable files, and errors gracefully.
"""
...
class CAISRealPipeline:
    """CAIS Pipeline with real error handling."""
    ...
    def __init__(self):
        ...
        print("\n" + "="*70)
        print(" CAIS - REAL PIPELINE (FIXED)")
    ...
    def _folder_has_files(self, category: str) -> bool:
        """Check if folder has downloadable files."""
        from modules.google_drive.connector import GoogleDriveConnector
    ...
    def run_step2_compress(self, directory_path: Optional[str]) -> Dict:
        if not directory_path:
            return {'success': False, 'error': 'No directory to compress'}
    ...
    def run_step4_build(self, config_path: str) -> Dict:
        ...
        return {'success': True, 'config': config, 'config_path': config_path}
    ...
    def run_pipeline(self, category: str, max_files: int = 5) -> Dict:
        ...
        if not download_result.get('success'):
            ...
            return {'success': False, 'error': 'Download failed', 'category': category}
        
        downloaded = download_result.get('downloaded', 0)
        if downloaded == 0:
            print(f"\n⚠️ No files downloaded from {category}")
            return {
                'success': False, 
        ...
        if not compress_result.get('success'):
            ...
            return {'success': False, 'error': 'Compression failed', 'category': category}
        ...
        if not analyze_result.get('success'):
            ...
            return {'success': False, 'error': 'Analysis failed', 'category': category}
        ...
        return {'success': True, 'duration': duration, 'report': str(report_file)}
    ...
    def discover_and_run(self, max_files: int = 3):
        ...
        for cat in categories[:10]:  # Process first 10
            print(f"\n{'='*70}")
            print(f" CHECKING: {cat}")
            print('='*70)
            
            # Check if folder has files
            if not self._folder_has_files(cat):
                print(f"⚠️ Skipping: {cat} - No downloadable files")
        ...
        self.generate_summary_report(processed, skipped)
    
    def generate_summary_report(self, processed: int, skipped: int):
        print("\n" + "="*70)
        ...
        if self.results:
            ...
            for r in self.results:
                print(f"   - {r.get('category', 'Unknown')}")
