...
class InstructionParser:
    ...
    def __init__(self, instructions_dir: str = "~/PROMETHEUS/downloads"):
        ...
        self.subtask_pattern = r'Subtarea\s+(\d+\.\d+)\s*[-:]\s*([^\n]+)'
        self.code_block_pattern = r'```(\w+)\n(.*?)```'
        self.acceptance_pattern = r'Criterios\s+de\s+Aceptaci[oó]n\.*?:?\s*([^\n]+)'
    ...
    def _parse_tasks(self, text: str, source_file: str):
        ...
        for match in task_matches:
            ...
            subtask_matches = re.findall(self.subtask_pattern, text, re.IGNORECASE)
            subtasks = []
            for subtask_match in subtask_matches:
                subtasks.append({
                    'number': subtask_match[0],
                    'description': subtask_match[1].strip()
                })

            acceptance_criteria = []
            acceptance_matches = re.findall(self.acceptance_pattern, text, re.IGNORECASE)
            for criteria in acceptance_matches:
                acceptance_criteria.append(criteria.strip())
    ...
    def _parse_agents(self, text: str, source_file: str):
        ...
        agent_pattern = r'Agent\s+([A-Za-z_]+)\s*[-:]\s*([^\n]+)([\s\S]*?)(?=\n\n|\n[A-Z]|\Z)'
    ...
    def _parse_code_blocks(self, text: str, source_file: str):
        ...
        code_matches = re.findall(self.code_block_pattern, text, re.DOTALL)
        for lang, code in code_matches:
            # Code blocks are stored with tasks or agents
            pass
